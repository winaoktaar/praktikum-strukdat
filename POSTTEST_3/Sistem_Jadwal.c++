...
string generateKode() {
    ...
    if (baseCount == 0) {
        ...
        return base + "-" + to_string(baseCount++);
    }
}
...
void addFirst(const string& tujuan, const string& status) {
    ...
    Node* nodeBaru = new Node{kode, tujuan, status, head, nullptr};
    if (head != nullptr) {
        head->prev = nodeBaru;
    } else {
        tail = nodeBaru; // kalau list kosong, tail ikut ke node baru
    }
    head = nodeBaru;
...
void addLast(const string& tujuan, const string& status) {
    string kode = generateKode();
    Node* nodeBaru = new Node{kode, tujuan, status, nullptr, tail};
    if (tail != nullptr) {
        tail->next = nodeBaru;
    } else {
        head = nodeBaru; // kalau list kosong
    }
    tail = nodeBaru;
...
void addMiddle(const string& tujuan, const string& status, int posisi) {
    ...
    if (temp->next == nullptr) { // sisip di akhir
        addLast(tujuan, status);
        return;
    }

    string kode = generateKode();
    Node* nodeBaru = new Node{kode, tujuan, status, temp->next, temp};
    temp->next->prev = nodeBaru;
    temp->next = nodeBaru;
    cout << "Disisipkan di posisi " << (hitung + 1) << ": " 
         << kode << " | " << tujuan << " | " << status << "\n";
}
...
void deleteFirst() {
    ...
    if (head != nullptr) {
        ...
        tail = nullptr; // kalau kosong total
    }
    cout << "Dihapus (head): " << hapus->kodePenerbangan 
         << " | " << hapus->tujuan 
         << " | " << hapus->status << "\n";
    delete hapus;
}
...
void updateStatusByKode(const string& kode, const string& newStatus) {
    ...
    while (temp != nullptr) {
        if (temp->kodePenerbangan == kode) {
            cout << "Sebelum: " << temp->kodePenerbangan << " | " 
                 << temp->tujuan << " | " << temp->status << "\n";
            temp->status = newStatus;
            cout << "Setelah:  " << temp->kodePenerbangan << " | " 
                 << temp->tujuan << " | " << temp->status << "\n";
            return;
        }
        temp = temp->next;
...
void display() {
    ...
    int idx = 1;
    cout << "+---------------------------------------------------------+\n";
    cout << "|              DAFTAR JADWAL PENERBANGAN (Depan->Belakang)|\n";
    cout << "+---------------------------------------------------------+\n";
    while (temp != nullptr) {
        cout << idx << ". " << temp->kodePenerbangan << " | Tujuan: " 
             << temp->tujuan << " | Status: " << temp->status << "\n";
        temp = temp->next;
        idx++;
    }
    cout << "+---------------------------------------------------------+\n";
...
void displayReverse() {
    ...
    int idx = 1;
    cout << "+---------------------------------------------------------+\n";
    cout << "|              DAFTAR JADWAL PENERBANGAN (Belakang->Depan)|\n";
    cout << "+---------------------------------------------------------+\n";
    while (temp != nullptr) {
        cout << idx << ". " << temp->kodePenerbangan << " | Tujuan: " 
             << temp->tujuan << " | Status: " << temp->status << "\n";
...
void showDetailByKodeOrNama(const string& key) {
    Node* temp = head;
    while (temp != nullptr) {
    ...
    cout << "Data dengan kunci \"" << key << "\" tidak ditemukan.\n";
}
...
void printMenu() {
    ...
    cout << " 2. Sisipkan Jadwal (posisi personal by NIM)\n";
    cout << " 3. Hapus Jadwal Paling Awal\n";
    cout << " 4. Update Status Penerbangan (by kode)\n";
    cout << " 5. Tampilkan Semua Jadwal (depan->belakang)\n";
    cout << " 6. Tampilkan Semua Jadwal (belakang->depan)\n";
    cout << " 7. Cari Detail Data (by kode atau tujuan)\n";
    cout << " 0. Keluar\n";
    cout << "+------------------------------------------------------------+\n";
    cout << "Pilih (0-7): ";
}
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 7: {
                ...
                getline(cin, key);
                if (key.empty()) { cout << "Input kosong. Batal.\n"; break; }
                showDetailByKodeOrNama(key);
                break;
            }
            default:
